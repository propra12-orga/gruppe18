...
public class Skull extends GameObject{
		...
		public Skull(int x, int y,String oid) {
			this.setOid(oid);
	...
	public void collisionWith(Object a) {
		if(a instanceof Player){
			SGameEngine.get().addAction(new PlayerDeadAction((Player)a));
		}
	}
	...
	static {
		...
		try {
			image = ImageIO.read(Bomberman.class.getClassLoader().getResource("resources/Totenkopf.png").openStream());
		} catch (IOException e) {
			e.printStackTrace();
		}
	}
